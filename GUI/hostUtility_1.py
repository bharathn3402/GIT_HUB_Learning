...
def mew_detect(startTimeForVCUhandshake,targetArduino):
    while (1):
        try:
            ...
            message = f"[{current_time}] - Arduino connection problem. Please restart utility."
    ...
    try:
        ...
        if arduinoResponse != b'':
            if arduinoResponse[0] == 0x51 and arduinoResponse[3] == 0x01:
                ...
                message = f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] - Mew handshake successful!"
                return communicationOkayFlag, numberOfDataSegments, message
            elif arduinoResponse[0] == 0x51 and arduinoResponse[3] == 0x10:
                communicationOkayFlag = 0
                message = f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] - Mew denied communication! Please restart the utility."
                return communicationOkayFlag, None, message
        else:
            message = f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] - Mew handshake timeout! Please restart the utility."
            communicationOkayFlag = None
            return communicationOkayFlag, None, message
    except Exception as error:
        message = f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] - Arduino connection problem. Please restart utility."
        raise SerialConnectionError(message)
