...
class HandshakeWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.uploadInProgressLabel.setFixedHeight(30)  # Set the height as desired
        self.uploadInProgressLabel.setAlignment(Qt.AlignCenter)
        self.uploadInProgressLabel.setTextFormat(Qt.RichText)  # Set text format to enable HTML formatting
        self.uploadInProgressLabel.adjustSize()
        self.uploadInProgressLabel.setVisible(False)
    ...
    def handle_arduino_handshake_result(self, arduino_handshake_flag,handshake_message):
        if arduino_handshake_flag == 1:
            ...
            message="Arduino Handshake successful!."
            QMessageBox.information(self, "Arduino handshake ", message)
            self.arduinohandshakeLabel.setVisible(False)
            ...
            if Handshake_box.clickedButton() == try_again_button:
                Handshake_box.done(QMessageBox.AcceptRole)
                self.reset()
            ...
            self.updateStatusLabel(handshake_message)
            QMessageBox.information(self, " connection problem", "Arduino connection problem. Please restart utility.")
            time.sleep(1)
            self.main_window.close()
    ...
    def handle_mew_handshake_result(self,mew_handshake_flag,number_of_data_segments,message):
        if mew_handshake_flag == 1:
            self.main_window.communicationOkayFlag = 1
            segments_message = f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] - Number of memory segments in hex file: {number_of_data_segments}"
            self.updateStatusLabel(message)
            self.updateStatusLabel(segments_message)
            message_1 = f"Mew handshake successful!\nNumber of memory segments in hex file: {number_of_data_segments}" 
            QMessageBox.information(self, "Mew handshake", message_1)
            self.handshakeInProgressLabel.setVisible(False)
            self.uploadInProgressLabel.setVisible(True)
            self.startButton.setVisible(True)

        elif mew_handshake_flag==0 and number_of_data_segments==None:
            self.main_window.communicationOkayFlag = 0
            message_1 = "Mew denied communication! Please restart the utility."
            self.updateStatusLabel(message)
            QMessageBox.information(self, "Mew denied communication", message_1)
            time.sleep(1)
            self.main_window.close()
        elif mew_handshake_flag==None and number_of_data_segments==None:
            self.updateStatusLabel(message)
            message_1 = "Mew handshake timeout! Please restart the utility."
            Handshake_box = QMessageBox(self)
            Handshake_box.setIcon(QMessageBox.Warning)
            Handshake_box.setWindowTitle("Mew handshake timeout!")
            Handshake_box.setText(message_1)
            try_again_button = Handshake_box.addButton("Try Again", QMessageBox.AcceptRole)
            exit_button = Handshake_box.addButton("Exit", QMessageBox.RejectRole)
            Handshake_box.exec_()
            ...
            if Handshake_box.clickedButton() == try_again_button:
                ...
                self.reset()
                self.main_window.activateWindow(2)
            ...
            message_1 = "Arduino connection problem. Please restart utility."
            self.updateStatusLabel(message)
            QMessageBox.information(self, "Mew handshake error", message_1)
            time.sleep(1)
            self.main_window.close()
...
class ArduinoHandshakeThread(QThread):
    ...
    def run(self):
        try:
            ...
            message = f"{current_time} - {str(error)} Please restart utility."
            self.arduino_handshake_signal.emit(None, message)
...
class MewDetectThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.mew_detect_signal.emit(None,message)

class MewHandshakeThread(QThread):
    mew_handshake_signal = pyqtSignal(int,int,str)
    ...
    def run(self):
        try:
            communicationOkayFlag, number_of_data_segments,message = hostUtility_1.mew_Handshake(self.hex_file, self.target_arduino)
            self.mew_handshake_signal.emit(communicationOkayFlag,number_of_data_segments,message)
        except hostUtility_1.SerialConnectionError as error:
            message = str(error) + " Please restart utility."
            self.mew_handshake_signal.emit(-1, None, message)
