...
while 1:
    ...
    if arduinoHandshakeFlag == 1:
                ...
                if ((time.time() - startTimeForVCUhandshake) < 60): #Check for VCU availability for 60 seconds
                    ...
                    if arduinoResponse != b'':
                        if arduinoResponse[0] == 0x51 and arduinoResponse[1] == 0x08:
                            bootloaderVersion = arduinoResponse[2]
        ...
        #Checksum validation
        firmwareChecksumUnique = False
        firmwareFlashedSuccessfully = False
        if bootloaderVersion > 0:
            print("\nChecking memory segment checksums of Mew.\n")
            for index, segment in enumerate((segmentList)): #reversed
                ...
                for byteAddress in range(segmentStartAddress, segmentEndAddress+1, 4):
                    ...
                    if (segmentEndAddress - byteAddress) >= 3:
                        tempSum = ((HexPyDictionary[byteAddress] << 24)
                ...
                CANdataBytes = [0x50, 0x08, 0xB0, 0x00] + [0x01 | ((index+1) << 4)] + [0x00]
                ...
                CANdataBytes = [0x50, 0x08, 0xB0, 0x00] + [0x02 | ((index+1) << 4)] + [0x00]
                ...
                CANdataBytes = [0x50, 0x08, 0xB0, 0x00] + [0x03 | ((index+1) << 4)] + [0x00]
                segmentChecksumBytes = segmentCheckSum.to_bytes(4, byteorder='big', signed=False)
                writtenBytes = sendArduinoCANframe(targetArduino, CANdataBytes, segmentChecksumBytes)

                print("\nMemory Segment #" + str(index+1) + " : " + hex(segmentStartAddress) + " - "+ hex(segmentEndAddress) + ", New FW Checksum: " + hex(segmentCheckSum))
                ...
                if arduinoResponse == b'':
                    ...
                    communicationOkayFlag = 0
                    break
                elif arduinoResponse[0] == 0x51 and arduinoResponse[3] == 0x02 and arduinoResponse[4] == (0x04 | ((index+1) << 4)):
                    vcuSegmentChecksum = (arduinoResponse[6] << 24
                                         | arduinoResponse[7] << 16
                                         | arduinoResponse[8] << 8
                                         | arduinoResponse[9])

                    print("Memory segment checksum mismatch! VCU Segment checksum: "+ hex(vcuSegmentChecksum) + ".\n\nProceeding to flash.\n")
                    ...
                    firmwareFlashedSuccessfully = False
                    firmwareChecksumUnique = True
                    break
                elif arduinoResponse[0] == 0x51 and arduinoResponse[3] == 0x01 and arduinoResponse[4] == (0x04 | ((index+1) << 4)):
        ...
        if firmwareChecksumUnique:
            if bootloaderVersion > 0:
                ...
                if arduinoResponse == b'':
                    ...
                    firmwareFlashedSuccessfully = False
                    communicationOkayFlag = 0
                elif arduinoResponse[0] == 0x51 and arduinoResponse[3] == 0x01 and arduinoResponse[4] == 0x02:
                    communicationOkayFlag = 1
            
            if communicationOkayFlag != 0:
                numberOfSegments = 0
    ...
    if firmwareFlashedSuccessfully:
        ...
        if communicationOkayFlag == 0:
            print("\nCommunication with Mew interrupted. Please restart the utility.\n")
        
        elif not firmwareChecksumUnique:
            print("\nVCU contains identical firmware. No need to reflash.\n")
